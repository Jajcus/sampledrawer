    ...
    def as_sql(self, columns=None, order_by="item.name", limit=100):
        ...
        for cond in self.conditions:
            if cond.applied_in_group:
                ...
                query = cond.get_sql_query(cond, cond_nr)
                cond_nr += 1
...
class MetadataQuery(SearchCondition):
    def __init__(self, key, value, oper="="):
        ...
        self.oper = oper
    def __repr__(self):
        return "<MetadataQuery %r %s %r>".format(self.key, self.oper, self.value)
    @classmethod
    def from_string(cls, query):
        for oper in ("=", "==", "<", ">", "<=", ">=", "!=", "<>"):
            if oper in query:
