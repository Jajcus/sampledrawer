...
class Library:
    ...
    def create_database(self, db_path):
        ...
        if os.path.exists(db_path):
            # safety check
            raise RuntimeError(
                "{!r} exists while not expected.".format(db_path))
        try:
            try:
    ...
    def import_file(self, metadata, copy=True):
        ...
        with self.db:
            cur = self.db.cursor()
            cur.execute(
                "SELECT id, name FROM items WHERE md5=? LIMIT 1", (md5,))
            row = cur.fetchone()
            if row is not None:
            ...
            values = [getattr(metadata, mdtype.name)
            ...
            for key in metadata:
                ...
                if row:
                    ...
                    cur.execute(
                        "INSERT INTO custom_keys(name) VALUES(?)", (key,))
